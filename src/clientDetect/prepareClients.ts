...
// client-detect の「検出入力」を datasets に事前用意する（npm install 的な setup。make prepare-clients）。
//   出力 clients.json … 依存メンバーシップ（どの版ペアで、どの client repo の、どのコミットを見るか）。
//                       state は一切含まない＝検出入力が結果フリーであることを監査可能。
//   採点の GT は元の datasets/test_result.json を直接参照する（派生コピーは作らない）。
// 版ペアは proposal_result（ライブラリ版遷移）、クライアントは test_result（S=依存 repo）から取る。
...
export async function runPrepareClients(): Promise<void> {
  ...
  for (const p of pairs) {
    ...
    for (const [client, verMap] of byC) {
      ...
      if (!atPrev || !atUpd) continue; // 両版に観測がある＝この遷移で評価可能な依存メンバー（state は問わない）
